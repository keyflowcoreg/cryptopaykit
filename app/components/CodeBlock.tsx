...
function highlightSyntax(code: string): string {
  return code
    ...
    .replace(/(\/\/.*$)/gm, '<span class="text-neutral-500 italic">$&</span>')
    // keywords
    .replace(
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-surface-700 bg-surface-800/50">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-neutral-600 uppercase">{language}</span>
          <button
            onClick={handleCopy}
            className="text-xs text-neutral-500 hover:text-amber-500 transition-colors px-2 py-1 rounded hover:bg-surface-700"
          >
            {copied ? 'Copied!' : 'Copy'}
          </button>
        </div>
